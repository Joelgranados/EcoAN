    ...
    if (~EOF),
      ...
      switch matchnum,
        case 1, [imgname]=strread(line,matchstrs(matchnum).str);
            ...
            % coinside.
            if strcmp(char(file_name), imgname) == 0
                % We have a problem.
                % FIXME: probably be a good idea to erase this file and
                % start a new one.  Lets error out for now.
                msgboxText{1} = strcat('There was a format error in file ', file_name);
                msgbox(msgboxText);
                break;
            end
        ...
        case 4, [obj,lbl,xmin,ymin,xmax,ymax]=strread(line,matchstrs(matchnum).str);
            annotation.regions(reg_off).label = char(lbl);
            annotation.regions(reg_off).bbox = [xmin,ymin,xmax,ymax];
            
            % prepare the next element for the next iteration.
            reg_off = reg_off + 1;
            annotation.regions(reg_off) = annotation_init;   
